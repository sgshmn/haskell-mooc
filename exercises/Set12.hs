...
instance Functor List where
  fmap _ Empty = Empty
  fmap f (LNode a bs) = LNode (f a) (fmap f bs)
...
count a bs = foldr (\x -> if x==a then (+1) else (+0)) 0 bs
...
inBoth xs ys = foldr (\x k -> if isIn x ys then x : k else k) [] xs
  where
    isIn x ys = foldr (\y k -> k || x == y) False ys
...
instance Foldable TwoList where
  foldr _ x TwoEmpty = x
  foldr f x (TwoNode a b c) = f a (f b (foldr f x c))
...
instance Functor Tree where
  fmap f Leaf = Leaf
  fmap f (Node x r l) = Node (f x) (fmap f r) (fmap f l)

sumTree :: Monoid m => Tree m -> m
sumTree (Node a r l) = todo
